...
- [x] S4 P4 Distributed replay ✋ — **done, merged.** `NonceLedger` extended with TTL + crash-recovery contract (no_std, `register_with_ttl`/`consume_at`); new `pqrascv-nonce-backends` crate with sync Redis (`SET NX EX`) + Postgres (`INSERT ON CONFLICT … WHERE expired`) reference impls, contract-tested vs in-memory fake; live-server tests feature/env-gated. Independently verified: 16 nonce + backend conformance tests pass, clippy clean. Atomicity reviewed.
- [x] S5 P5 PKI ✋ — **done, merged (`07709aa`).** Trait-only `KeyProvider`/HSM abstraction + `SoftwareKeyProvider`; `OverlapWindow` dual-trust rollover, trust-anchor rotation, revocation carry-forward; `PKI_OPERATIONS.md`. Independently verified: 48 pki tests pass, clippy clean.
- [~] S6 P6 keyd ✋ — **merged (`eaef24c`), verification partial.** Per-key ACLs (default-deny), injectable audit log, usage tracking, pure socket-free `Authorizer`; Unix-socket-only, additive `Denied` status. Logic reviewed (default-deny + decision flow sound). **NOTE:** keyd is `#[cfg(unix)]`; on this Windows host the unix tests cannot RUN — cross-clippy (Linux target) verifies compile+lint+test-compile; test EXECUTION deferred to Linux CI.
- [x] S6 P7 7A+7B ✋ — **done, merged (`33872fb`).** Decisions: rust-bitcoin 0.32, new lifecycle in dir `crates/pqrascv-bitcoin-anchor` as package `pqrascv-bitcoin-broadcast` (the core crate already owns the `pqrascv-bitcoin-anchor` package name). 7A `AnchorBuilder`: non-custodial unsigned-tx construction, OP_RETURN-last, deterministic, artifact-agnostic helpers. 7B `Broadcaster` trait + bounded transient-only retry + idempotency contract; Core-RPC/Esplora/Electrum live impls behind `live-network`; always-on `MockBroadcaster`. Independently verified: 21 tests pass, clippy clean (both feature sets). builder/broadcast logic reviewed.
- [x] S7 P7 7C+7D ✋ — **done, committed (`df4ab55`), implemented INLINE.** 7C `HeaderOracle` + `validate_chain` (local PoW / continuity / within-period difficulty stability via rust-bitcoin); `FixtureHeaderOracle`. 7D `MultiSourceHeaderOracle` (quorum cross-check, divergent/eclipse flagging, fail-closed on split). 11 tests, clippy clean. **Dispatch note:** the 7C+7D sub-agent got a no-shell sandbox AND a worktree missing the 7A+7B base (worktrees branch from a stale point) → could not build. Remaining Phase 7 stages are sequential/dependent, so they are being done inline, not via worktree dispatch.
- [x] S8 P7 7E+7F+7G ✋ — **done (inline).** 7E `AnchorLifecycle` (create/broadcast/track/wait/verify; verify_anchor binds proofs to the oracle-validated chain). 7F `FederationCheckpoint` (chained governance history), 7G `ProvenanceAnchor` (firmware-identity finality), domain-separated SHA3-256 commitments via `Anchorable`/`create_anchor_for`. 46 crate tests, clippy clean.
- [x] S9 P7 7H ✋ — **done.** `BITCOIN_FINALITY_ARCHITECTURE.md`: verification/anchoring paths, trust assumptions, attack surfaces (with honest known gaps: difficulty-retarget value, live-network contract-only, idempotency heuristics, BIP157/158 absent), operational model, and benchmarks vs OpenTimestamps / CT / Sigstore / enterprise retention. Readiness: logic Tier-1; operational sign-off pending live-node + real-vector validation.
- [x] S10 P8 Re-audit ✋ — **done.** `ENTERPRISE_READINESS_V2.md`: per-subsystem Technical/Security/Operational/Enterprise scores from code+tests (no speculation). Evidence gathered via 2 read-only Explore agents (verifier/sigstore, ffi/python/cli) + first-hand P1–P7 knowledge. Findings: strategic core is Production-Ready in logic/security; uniform Enterprise sign-off gated on operational (observability) + empirical (live-env/vendor-vector) validation, not architecture. Test inventory: 580+ unit tests, 4 Kani proofs, 7 fuzz targets. Also fixed a stale `policy_federation` module doc that predated the Phase 2 signed-approval work.
